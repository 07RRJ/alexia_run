...
for runing in range(50):
    ...
    for i in range(100):
        ...
        # Filter: Only platforms close to the screen are checked and drawn
        visible_platforms = [
            p for p in platforms if -1000 < p.x < SCREEN_WIDTH + 200
        ]
        ...
        # Cap steps to avoid lag spike
        max_steps = 5
        steps = min(int(abs(y_vel)) + 1, max_steps)
        for _ in range(steps):
            player_y += y_vel / steps
        ...
        # Collision detection (vertical, visible only)
        for platform in visible_platforms:
